...
from .config_io import template_text
...
def run_init(config_file: Path, args):
    ...
    静默模式渲染共享模板（config_io.template_text），结合 -i 可进入交互式引导模式。
    """
    ...
    # 静默模式：统一渲染 config_io.template_text()。此前优先复制
    # jenkins-config.example.yaml/json，导致同一份"初始模板"有三个来源
    # （示例文件、内联表头 + generate_template、MCP init_config），
    # 改一处字段就得记得改另外两处；EXE / npx 形态下示例文件本就不存在。
