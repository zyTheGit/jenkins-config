...
- npx distribution: `npm/` is a Node launcher package published as `@zythegit/jenkins-config-mcp` (`npx -y @zythegit/jenkins-config-mcp`) that downloads the platform binary from GitHub Release on first run (sha256-verified against `checksums.txt`, cached under `~/.cache/jenkins-config-mcp/<tag>/`), so consumers need only Node 18+. Resolution order: `JENKINS_MCP_BINARY` → `JENKINS_MCP_PYTHON` → cached binary → download → PATH `jenkins-config-mcp`/`uvx`/`python`. All launcher logs go to stderr; stdout is the JSON-RPC channel. A resolved command ending in `.cmd` / `.bat` (the Windows pip/npm console-script shim, i.e. exactly what the fallback branch hits) is forwarded through `COMSPEC` (default `cmd.exe`) with `/d /s /c` and cross-spawn-style escaping instead of being spawned directly — Node ≥18.20 / 20.12 throws `EINVAL` on batch files under `shell: false` (CVE-2024-27980), while `shell: true` is avoided because Node 24 deprecates passing args that way (DEP0190) and concatenates them unescaped; **argument** meta chars are escaped twice (a shim's `%*` gets re-parsed) while the command path is escaped once. Everything else keeps `shell: false`, and the dry-run output reports the decision as `via_cmd`. On that cmd.exe path two things follow from `child` being cmd.exe rather than the server: `SIGINT` / `SIGTERM` are delivered as `taskkill /PID <pid> /T /F` (once only — the taskkill path never sets `child.killed`, so a repeat signal would re-kill a possibly-recycled PID — falling back to `child.kill` both when taskkill can't spawn and when it exits non-zero) because killing cmd.exe alone leaves the real server orphaned holding the inherited stdio; a self-initiated shutdown then exits 0 instead of passing through the non-zero code `/F` leaves on cmd.exe, and the signal listener is removed before re-raising so the launcher doesn't catch its own signal. A missing shim is pre-checked with `which()` (not bare `fs.existsSync` — the command may be a bare name that cmd.exe would resolve via PATH; `which()` in turn tries a name verbatim when it already carries a PATHEXT suffix, otherwise `foo.cmd` would only ever be probed as `foo.cmd.EXE`) since cmd.exe turns it into a plain exit code 1 instead of a spawn `'error'` event
